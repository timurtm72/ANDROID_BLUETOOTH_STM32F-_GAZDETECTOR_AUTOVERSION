#line 1 "D:/ALL_PROJECTS/GAZ_DETECTOR/UZCAR_STM32F0/UZCAR_STM32F031_MC101_BTU/base_mq.c"
...
#line 3 "D:/ALL_PROJECTS/GAZ_DETECTOR/UZCAR_STM32F0/UZCAR_STM32F031_MC101_BTU/base_mq.c"
double calculatePPM(double adcValue,double R0,double cal_data)
{
 return (double) map((double)adcValue,(double)R0,(double)cal_data,0, 10000 );
}
...
float getR0(float Rl,float adcValue,float koef, float volt, float adcRange)
{
 ...
 float Rs;
 float R0;
 Vrl = (float)adcValue * (volt / adcRange);
 Rs = (((volt - Vrl) / Vrl) * Rl);
 R0 = (Rs / koef);
 return R0;
}
